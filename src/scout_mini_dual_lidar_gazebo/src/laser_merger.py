...
class LaserMerger(Node):
    def __init__(self):
        ...
        self.rear_sub = self.create_subscription(
            LaserScan, '/rear/scan_fixed', self.rear_callback, 10)

        self.merged_pub = self.create_publisher(LaserScan, '/merged/scan', 10)
        self.get_logger().info('Merging /front/scan + /rear/scan -> /merged/scan (TF-corrected + hull-filtered)')
    ...
    def try_publish(self):
        ...
        for i in range(n):
            ...
            fr = self._correct_and_filter(fs.ranges[i], cos_a, sin_a, self.front_tf)
            rr = self._correct_and_filter(rs.ranges[i], cos_a, sin_a, self.rear_tf)
