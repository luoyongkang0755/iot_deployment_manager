#!/usr/bin/env python3
"""
Nav2 Simulation Launch — Gazebo + robot + Nav2 in one launch file.
...
def generate_launch_description():
    ...
    declare_params_file = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(pkg_nav2, 'config', 'simulation', 'nav2_params.yaml'),
        description='Full path to Nav2 params file')

    declare_world = DeclareLaunchArgument(
        'world', default_value=os.path.join(pkg_nav2, 'worlds', 'simple_test_world.world'),
        description='Full path to the world model file to load')

    declare_model = DeclareLaunchArgument(
        'model', default_value=os.path.join(pkg_nav2, 'urdf', 'scout_mini_gazebo.xacro'),
    ...
    declare_spawn_yaw = DeclareLaunchArgument(
        ...
        description='Initial yaw angle for robot spawn')
    ...
    gazebo = IncludeLaunchDescription(
        ...
        launch_arguments={'gz_args': ['-v 4 ', world] if verbose else world}.items())

    spawn_entity = Node(
        package='ros_gz_sim', executable='create',
        arguments=['-name', 'scout_mini', '-topic', 'robot_description',
                   '-x', '0.0', '-y', '0.0', '-z', '0.181', '-Y', spawn_yaw],
        output='screen')

    # ROS-Gazebo bridges (SIMULATION ONLY)
    gz_bridge = Node(
    ...
    # Simulation-specific sensor processing (SIMULATION ONLY)
    scan_frame_fixer = Node(
        package='scout_mini_dual_lidar_gazebo', executable='scan_frame_fixer.py',
        name='scan_frame_fixer', output='screen',
        parameters=[{'use_sim_time': use_sim_time}])

    imu_odom_corrector = Node(
        package='scout_mini_dual_lidar_gazebo', executable='imu_odom_corrector.py',
        name='imu_odom_corrector', output='screen',
        parameters=[{'use_sim_time': use_sim_time}])

    odom_to_tf = Node(
        package='scout_mini_dual_lidar_gazebo', executable='odom_to_tf.py',
        name='odom_to_tf', output='screen',
        parameters=[{'use_sim_time': use_sim_time}])

    laser_merger = Node(
        package='scout_mini_dual_lidar_gazebo', executable='laser_merger.py',
        name='laser_merger', output='screen',
        parameters=[{'use_sim_time': use_sim_time}])
